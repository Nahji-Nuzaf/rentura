...
export default function LoginPage() {
  ...
  const handleLogin = async () => {
    ...
    const { data, error: signInError } = await sb.auth.signInWithPassword({ email: email.trim(), password })
    ...
    <>
      <style>{`
        ...
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
        ...
        .submit{width:100%;padding:15px;border-radius:13px;border:none;background:linear-gradient(135deg,#0EA5E9,#6366F1);color:#fff;font-size:15px;font-weight:700;cursor:pointer;font-family:'Plus Jakarta Sans',sans-serif;box-shadow:0 6px 24px rgba(14,165,233,.35);transition:all .25s;position:relative;overflow:hidden;letter-spacing:.2px}
        .submit::after{content:'';position:absolute;inset:0;background:linear-gradient(135deg,rgba(255,255,255,.15),transparent)}
        ...
        @media(max-width:900px){
          ...
          .left{flex:none;padding:36px 28px 40px}
          .right{flex:none;padding:36px 20px 48px}
          .orb{width:200px;height:200px;top:-30px;right:-30px}
          .orb2{display:none}
        ...
        <div className="right">
          <div style={{ width:'100%', maxWidth:400, position:'relative', zIndex:1 }}>
            ...
            <div style={{ marginBottom:28 }}>
              <div style={{ width:52, height:52, borderRadius:15, background:'linear-gradient(135deg,#38BDF8,#6366F1)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:24, marginBottom:20, boxShadow:'0 12px 32px rgba(56,189,248,.25)' }}>🏘️</div>

...
export default function TenantDashboard() {
  ...
  useEffect(() => {
    ;(async () => {
      try {
        ...
        // Property
        const { data: propData } = await sb.from('properties').select('*').eq('id', tRow.property_id).single()
        if (propData) setProperty(propData)

        // Landlord profile — get landlord_id from property
        ...
        if (senderIds.length) {
          const { data: senders } = await sb.from('profiles').select('id,full_name').in('id', senderIds)
  ...
  }[currentPayStatus] ?? { label: '—', bg: '#F1F5F9', color: '#64748B' }
    ...
    <>
      <style>{`
        ...
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
        ...
        /* ── Hero ── */
        .hero{background:linear-gradient(135deg,#0F172A 0%,#1E293B 55%,#1a3354 100%);border-radius:20px;padding:28px 32px;margin-bottom:22px;display:flex;align-items:center;justify-content:space-between;position:relative;overflow:hidden;flex-wrap:wrap;gap:20px}
        ...
        .hero-name{font-family:'Fraunces',serif;font-size:28px;font-weight:700;color:#fff;margin-bottom:10px}
        ...
        .hero-rent{font-family:'Fraunces',serif;font-size:36px;font-weight:700;color:#fff;line-height:1}
        ...
        .qa-card{background:#fff;border:1.5px solid #E2E8F0;border-radius:14px;padding:18px 14px;text-align:center;cursor:pointer;transition:all .18s;box-shadow:0 1px 4px rgba(15,23,42,.04);text-decoration:none;display:block}
        ...
        .qa-label{font-size:12.5px;font-weight:700;color:#475569}
        ...
        /* Rent card */
        .rent-amount{font-family:'Fraunces',serif;font-size:38px;font-weight:700;color:#0F172A;margin-bottom:4px}
        .rent-meta{font-size:13px;color:#64748B;margin-bottom:18px}
        .pay-btn{width:100%;padding:13px;border-radius:12px;border:none;background:linear-gradient(135deg,#2563EB,#6366F1);color:#fff;font-size:14px;font-weight:700;cursor:pointer;font-family:'Plus Jakarta Sans',sans-serif;box-shadow:0 4px 14px rgba(37,99,235,.3);transition:opacity .15s}
        ...
        /* Lease bar */
        .lease-dates{display:flex;justify-content:space-between;font-size:12px;color:#64748B;margin-bottom:8px}
        .lease-bar-bg{height:8px;background:#E2E8F0;border-radius:99px;overflow:hidden;margin-bottom:7px}
        .lease-bar-fill{height:100%;border-radius:99px;background:linear-gradient(90deg,#2563EB,#6366F1)}
        .lease-pct-row{display:flex;justify-content:space-between;font-size:12px;margin-bottom:14px}
        ...
        .tl-row{display:flex;align-items:center;gap:10px;padding:11px 12px;background:#F8FAFC;border:1px solid #E2E8F0;border-radius:11px}
        ...
        /* Maintenance mini */
        .maint-row{display:flex;align-items:center;gap:10px;padding:11px 12px;border:1px solid #E2E8F0;border-radius:11px;margin-bottom:8px}
        ...
        /* Messages mini */
        .msg-row{display:flex;gap:10px;padding:12px;background:#F8FAFC;border:1px solid #E2E8F0;border-radius:12px;margin-bottom:8px}
        .msg-av{width:38px;height:38px;border-radius:10px;background:linear-gradient(135deg,#2563EB,#6366F1);display:flex;align-items:center;justify-content:center;color:#fff;font-size:12px;font-weight:700;flex-shrink:0}
        .msg-from{font-size:13px;font-weight:700;color:#0F172A}
        .msg-text{font-size:12px;color:#64748B;margin-top:2px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:180px}
        ...
        .modal{background:#fff;border-radius:22px;padding:32px 28px;width:100%;max-width:380px;box-shadow:0 24px 60px rgba(15,23,42,.2);text-align:center}
        .modal-icon{font-size:44px;margin-bottom:14px}
        .modal-title{font-family:'Fraunces',serif;font-size:24px;font-weight:700;color:#0F172A;margin-bottom:8px}
        ...
        @media(max-width:768px){
          ...
          .main{margin-left:0!important;width:100%!important;padding-top:0}
          ...
          <div className="content">
            ...
            {!tenantRow ? (
              <div className="no-tenant">
                ...
                <div className="no-tenant-sub">You don't have an active tenancy linked to your account.<br />Contact your landlord to get an invite code.</div>
              </div>
            ) : (
              <>
                {/* Hero */}
                <div className="hero">
                  <div>
                    ...
                    <div className="hero-chips">
                      ...
                      {unit && <span className="hero-chip">🚪 Unit {unit.unit_number}</span>}
                  ...
                  <div className="hero-right">
                    ...
                    {daysLeft !== null && (
                      <div className="hero-pill" style={{
                        background: daysLeft <= 0 ? 'rgba(220,38,38,.15)' : daysLeft <= 3 ? 'rgba(220,38,38,.15)' : 'rgba(251,191,36,.15)',
                ...
                <div className="stats">
                  ...
                  <div className="stat-card">
                    <div className="stat-icon">🔧</div>
                    <div className="stat-val" style={{ color: openMaint > 0 ? '#D97706' : '#16A34A' }}>
                      {openMaint}
                    </div>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                    ...
                    <div className="card">
                      ...
                      <div className="rent-meta">
                        Due day {unit?.rent_due_day} ·{' '}
                        {daysLeft !== null && daysLeft > 0 ? `${daysLeft} days away` : daysLeft === 0 ? 'Due today' : 'Overdue'}
                      </div>
                      <button
                        className={`pay-btn${currentPayStatus === 'paid' ? ' paid' : ''}`}
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                    ...
                    <div className="card">
                      ...
                      {recentPayments.length === 0 ? (
                        <div style={{ fontSize: 13, color: '#94A3B8', textAlign: 'center', padding: '24px 0' }}>No payment records yet</div>
                      ) : (
                        <div className="tl">
                    ...
                    <div className="card">
                      ...
                      {maintenance.length === 0 ? (
                        <div style={{ fontSize: 13, color: '#94A3B8', textAlign: 'center', padding: '24px 0' }}>No requests yet</div>
                      ) : (
                    ...
                    <div className="card">
                      <div className="card-header">
                        <div className="card-title">Messages {unreadCount > 0 && <span style={{ background: '#DC2626', color: '#fff', fontSize: 10, fontWeight: 700, padding: '1px 6px', borderRadius: 99, marginLeft: 6 }}>{unreadCount}</span>}</div>
                        <a href="/tenant/messages" className="card-action">View All →</a>
                      </div>
                      {messages.length === 0 ? (
                        <div style={{ fontSize: 13, color: '#94A3B8', textAlign: 'center', padding: '24px 0' }}>No messages yet</div>
                      ) : (

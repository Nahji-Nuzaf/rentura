...
// units.status → occupied | vacant | maintenance
// units.rent_due_day → 1–28
...
export default function UnitsPage() {
  ...
  useEffect(() => {
    ...
    const init = async () => {
      try {
        ...
        // Flat query: load tenants for these units
        let tenantMap: Record<string, { name: string; email: string }> = {}
        if (unitIds.length > 0) {
  ...
  async function handleSaveUnit() {
    ...
    if (isNaN(rent) || rent < 0) { setSaveError('Please enter a valid rent amount.'); return }
    const dueDay = parseInt(editForm.rent_due_day)
    if (isNaN(dueDay) || dueDay < 1 || dueDay > 28) { setSaveError('Rent due day must be between 1 and 28.'); return }
  ...
  async function applyBulk() {
    ...
    try {
      ...
      const { error } = await supabase.from('units').update(update).in('id', ids)
      if (error) throw error

      setUnits(prev => prev.map(u =>
        ids.includes(u.id) ? { ...u, ...update } : u
      ))
      setSelected(new Set())
      setBulkRent('')
      setBulkStatus('')
    ...
    <>
      <style>{`
        ...
        *,*::before,*::after{margin:0;padding:0;box-sizing:border-box}
        ...
        .btn-primary{padding:8px 18px;border-radius:10px;border:none;background:linear-gradient(135deg,#2563EB,#6366F1);color:#fff;font-size:13px;font-weight:700;cursor:pointer;font-family:'Plus Jakarta Sans',sans-serif;box-shadow:0 2px 10px rgba(37,99,235,.28);transition:all .18s}
        ...
        .prop-badge{display:inline-flex;align-items:center;gap:6px;padding:4px 12px;background:#EFF6FF;border:1px solid #BFDBFE;border-radius:99px;font-size:12px;font-weight:600;color:#2563EB;margin-bottom:10px;max-width:100%;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
        ...
        .sstat-txt{min-width:0;overflow:hidden}
        .sstat-num{font-family:'Fraunces',serif;font-size:20px;font-weight:700;color:#0F172A;line-height:1;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
        ...
        .ftab{padding:7px 14px;border-radius:8px;font-size:13px;font-weight:600;cursor:pointer;border:none;background:none;color:#64748B;font-family:'Plus Jakarta Sans',sans-serif;transition:all .15s;display:inline-flex;align-items:center;gap:6px;white-space:nowrap;flex-shrink:0}
        ...
        .units-table th{padding:12px 14px;text-align:left;font-size:11px;font-weight:700;color:#64748B;text-transform:uppercase;letter-spacing:.6px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
        ...
        .preset-ico{font-size:18px;display:block;margin-bottom:3px}
        .preset-lbl{font-size:11.5px;font-weight:600;color:#475569}
        ...
        @media(max-width:768px){
          ...
          .units-table-wrap{display:none!important}
          .unit-cards{display:flex!important}
          /* Bulk bar stack */
          .bulk-bar{flex-direction:column;align-items:stretch}
          .bulk-field{width:100%}
          .bulk-input,.bulk-select{flex:1;width:100%}
        }
      ...
      <div className={`drawer${editUnit ? ' open' : ''}`}>
        ...
        <div className="drawer-body">
          <div className="field">
            ...
            <input placeholder="e.g. Unit 5A, Penthouse 1" value={editForm.unit_number}
          ...
          <div style={{fontSize:'12px',color:'#94A3B8'}}>
            💡 Quick rent presets by unit type:
          </div>
          <div className="preset-grid">
            {BEDROOM_PRESETS.map(p => (
              <button key={p.label} className="preset-btn"
                onClick={() => {
                  const baseName = editForm.unit_number.replace(/\s*\(.*\)$/, '')
                  setEditForm(f => ({
                    ...f,
                    unit_number:  `${baseName} (${p.label})`,
                    monthly_rent: String(p.rent),
                  }))
                }}>
                <span className="preset-ico">{p.icon}</span>
                <span className="preset-lbl">{p.label}</span>
                <span style={{fontSize:'10px',color:'#94A3B8',marginTop:'2px'}}>${p.rent}/mo</span>
              </button>
            ))}
          </div>
        ...
        <div className="main">
          <div className="topbar">
            <div className="tb-left">
              ...
              <div className="breadcrumb" style={{overflow:'hidden',whiteSpace:'nowrap',textOverflow:'ellipsis',maxWidth:'calc(100vw - 200px)'}}>
                <a href="/landlord/properties">Properties</a>
                <span style={{margin:'0 4px',color:'#CBD5E1'}}>/</span>
                <a href={`/landlord/properties`} style={{maxWidth:120,overflow:'hidden',textOverflow:'ellipsis',display:'inline-block',verticalAlign:'bottom'}}>{property?.name || '…'}</a>
                <span style={{margin:'0 4px',color:'#CBD5E1'}}>/</span>
                <b>Units</b>
              </div>
            </div>
            <div className="tb-actions">
              {!bulkMode ? (
                <button className="btn-outline" onClick={()=>{setBulkMode(true);setSelected(new Set())}}>
                  ✏️ <span className="tb-label">Bulk Edit</span>
                </button>
              ) : (
                <button className="btn-outline" onClick={()=>{setBulkMode(false);setSelected(new Set())}}>
                  ✕ <span className="tb-label">Cancel</span>
                </button>
              )}
              <a href="/landlord/properties" className="btn-outline">
                ← <span className="tb-label">Back</span>
              </a>
            </div>
          </div>
          ...
          <div className="content">
            ...
            <div className="page-header">
              ...
              <div className="page-sub">
                Set individual rents, statuses, and lease dates for each unit
              </div>
            </div>
            ...
            <div className="stat-strip">
              <div className="sstat">
                <div className="sstat-ico" style={{background:'#EFF6FF'}}>🏗️</div>
                <div className="sstat-txt"><div className="sstat-num">{counts.all}</div><div className="sstat-lbl">Total Units</div></div>
              </div>
              <div className="sstat">
                <div className="sstat-ico" style={{background:'#DCFCE7'}}>✅</div>
                <div className="sstat-txt"><div className="sstat-num">{counts.occupied}</div><div className="sstat-lbl">Occupied</div></div>
              </div>
              <div className="sstat">
                <div className="sstat-ico" style={{background:'#FEF3C7'}}>🔑</div>
                <div className="sstat-txt"><div className="sstat-num">{counts.vacant}</div><div className="sstat-lbl">Vacant</div></div>
              </div>
              <div className="sstat">
                <div className="sstat-ico" style={{background:'#DCFCE7'}}>💰</div>
                <div className="sstat-txt">
                  <div className="sstat-num">${totalRevenue.toLocaleString()}</div>
            ...
            <div className="toolbar">
              ...
              <div className="search-wrap">
                ...
                <input className="search-input" placeholder="Search unit or tenant…"
            ...
            {bulkMode && selected.size > 0 && (
              <div className="bulk-bar">
                ...
                <div className="bulk-field">
                  ...
                  <input className="bulk-input" type="number" placeholder="e.g. 600"
                ...
                <button className="bulk-apply" disabled={bulkSaving || (!bulkRent && !bulkStatus)} onClick={applyBulk}>
            ...
            {loading ? (
              <div className="units-table-wrap">
                <table className="units-table">
                  <thead><tr>
                    <th>Unit</th><th>Tenant</th><th>Rent</th><th>Status</th><th>Lease</th><th></th>
                  </tr></thead>
                  <tbody>
                    {[1,2,3,4,5].map(i => (
                      <tr key={i}>
                  ...
                  <tbody>
                    {filtered.map(u => {
                      ...
                      if (u.lease_start && u.lease_end) {
                        const s = new Date(u.lease_start).getTime()
                        const e = new Date(u.lease_end).getTime()
                        const n = Date.now()
                        leasePct = Math.min(100, Math.max(0, Math.round(((n - s) / (e - s)) * 100)))
                      }
                      return (
                        <tr key={u.id} className={selected.has(u.id) ? 'selected-row' : ''}>
                          {bulkMode && (
                            ...
                            {u.lease_start && u.lease_end ? (
                              <div>
                                <div className="lease-dates">
                                  {new Date(u.lease_start).toLocaleDateString('en-US',{month:'short',year:'numeric'})}
                                  {' → '}
                                  {new Date(u.lease_end).toLocaleDateString('en-US',{month:'short',year:'numeric'})}
                                </div>
                                <div className="lease-bar-wrap">
                                  <div className="lease-bar">
                                    <div className="lease-fill" style={{width:`${leasePct}%`}} />
                                  </div>
                                </div>
                              </div>
                            ) : (
            ...
            {!loading && (
              <div className="unit-cards">
                {filtered.length === 0 ? (
                  ...
                  const leaseColor = leasePct > 80 ? '#EF4444' : leasePct > 60 ? '#F59E0B' : '#3B82F6'
                  return (
                    <div key={u.id} className="unit-card">
                      {/* Top row: unit number + status badge */}
                      <div className="uc-top">
                        <span className="uc-num">{u.unit_number}</span>
                        <span className="uc-badge" style={{background:sc.bg,color:sc.color}}>
                          ● {sc.label}
                        </span>
                      </div>

                      {/* Body: rent + lease side by side */}
                      <div className="uc-body">
                        ...
                        <div className="uc-field">
                          ...
                          {u.lease_start && u.lease_end ? (
                            <>
                              <div className="uc-sub" style={{marginTop:2}}>
                                {new Date(u.lease_start).toLocaleDateString('en-US',{month:'short',year:'numeric'})}
                              </div>
                              <div className="uc-sub">
                                → {new Date(u.lease_end).toLocaleDateString('en-US',{month:'short',year:'numeric'})}
                              </div>
                              <div style={{height:4,background:'#E2E8F0',borderRadius:99,overflow:'hidden',marginTop:5,width:'100%'}}>
                                <div style={{height:'100%',width:`${leasePct}%`,background:leaseColor,borderRadius:99,transition:'width .4s'}}/>
                              </div>
                            </>
                          ) : (
                      ...
                      <div className="uc-footer">
                        ...
                        <button className="edit-btn" style={{flexShrink:0}} onClick={()=>openEdit(u)}>Edit</button>
                      </div>
                    </div>
                  )
                })}

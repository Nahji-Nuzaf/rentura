...
export default function PropertiesPage() {
  ...
  function openEdit(p: Property) {
    setForm({
      ...
      default_rent: p.avg_rent > 0 ? String(p.avg_rent) : '',
    })
    setEditing(p)
    setSaveError(null)
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (drawer === 'add') {
        // 1. Insert property first (need id for photo upload)
        ...
        // For edit: new files get uploaded, existing URLs kept from previews
        setUploadingPhotos(true)
        const newUrls = await uploadPhotos(editing.id)
        setUploadingPhotos(false)
        // Existing photos = previews that are URLs (not data:// blobs)
        ...
        const newRent = parseFloat(form.default_rent)
        if (!isNaN(newRent) && newRent > 0) {
          await supabase.from('units').update({ monthly_rent: newRent }).eq('property_id', editing.id)
        }
    ...
    <>
      <style>{`
        ...
        *,*::before,*::after{margin:0;padding:0;box-sizing:border-box}
        ...
        .plan-bar-fill{height:100%;border-radius:99px;background:linear-gradient(90deg,#3B82F6,#6366F1);transition:width .4s}
        .plan-bar-count{font-size:12.5px;font-weight:700;color:#0F172A}
        ...
        /* BANNER — shows photo or gradient */
        .prop-banner{height:150px;position:relative;flex-shrink:0;overflow:hidden;background:linear-gradient(135deg,#EFF6FF,#EEF2FF)}
        ...
        .prop-thumb{width:36px;height:36px;border-radius:7px;object-fit:cover;border:2px solid rgba(255,255,255,0.8);box-shadow:0 1px 4px rgba(0,0,0,0.15);cursor:pointer}
        ...
        @media(max-width:480px){
          .stat-strip{grid-template-columns:repeat(2,1fr)}
          .sstat{padding:12px 14px;gap:10px}
          .sstat-num{font-size:22px}
          .filter-tabs{gap:3px}
          .ftab{padding:6px 10px;font-size:12px}
      ...
      <div className={`drawer${drawer ? ' open' : ''}`}>
        ...
        <div className="drawer-body">
          ...
          <div className="hint">
            {drawer === 'add'
              ? '💡 All units will be created automatically. Go to Manage Units on the card to set individual rents per unit.'
              : '💡 Changing the default rent will update all units. Use Manage Units to set individual rents.'}
          </div>
